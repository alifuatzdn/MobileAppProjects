...
export default function editCreditCard() {
  ...
  const fetchCreditCard = async () => {
    try {
      const card = await getCreditCardById(Number(id));
      setFormData({
        name: card.name || "",
        cardNumber: card.cardNumber || "",
        cardHolderName: card.cardHolderName || "",
        validationDate: card.validationDate
          ? card.validationDate.slice(5, 7) + "/" + card.validationDate.slice(2, 4) : "",
        securityCode: card.securityCode || 0,
      });
    } catch (error) {
      console.error("Kart bilgileri alınırken bir hata oluştu.");
    }
  };
  ...
  const handleSaveCard = async () => {
    try {
      ...
      console.log("Gönderilen Veri:", updatedFormData);
      await updateCreditCard(Number(id), updatedFormData);
      Alert.alert("Kart başarıyla güncellendi.");
      router.back()
    } catch (error) {
    ...
    >
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerText}>Kredi Kartı Düzenle</Text>
        </View>
